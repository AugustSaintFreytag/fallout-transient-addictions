...
A mod for Fallout: New Vegas / Tale of Two Wastelands that introduces a time and chance based natural recovery from addictions. If the player is in good health and has not taken the item in a few days, there is a random chance (based on endurance and luck) to heal an addiction on its own. Written in NVSE, ESPLess, safe to add or remove at any time.

## About

For each active addiction, there's a chance to naturally recover. The player needs to be healthy and has to abstain from using any items giving the addiction effect for at least a couple of days. Endurance and luck stats increase the chances for a recovery. Active withdrawal effects are checked every tick (every 30 seconds by default, configurable). Addictions cannot be healed while being suppressed by an item giving them. Once an addiction is recovered, you will be informed via congratulatory message. The message(s) will be queued up and displayed when not in combat. 
